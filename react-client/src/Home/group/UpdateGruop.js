...
export class UpdateGroup extends React.Component {
    constructor(props) {
        ...
        this.state = { CreateYear: new Date(this.props.location.state.CreateYear).toISOString().substring(0,10), Name: this.props.location.state.Name, Specialty: this.props.location.state.Specialty, isLoad: false, CreateYearError: false, NameError: false, SpecialtyError: false, addState: 0, isUnAuth: false }
    ...
    addFunc(e) {
        ...
        if ((this.state.Name.length === 0 ? true : false) === false & (this.state.Specialty.length === 0 ? true : false) === false & (this.state.CreateYear === null ? true : false) === false) {
            fetch('https://localhost:5001/api/groups/'+this.props.location.state.Id, {
                method: 'PUT',
                headers: {
